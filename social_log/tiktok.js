...
module.exports = client => {
  client.Jobtiktok = new CronJob('0 */7 * * * *', async function() {
    ...
    var guilds = client.social_log.keyArray(v => 
      v.tiktok && v.tiktok.channels && v.tiktok.channels.length > 0 &&
      v.tiktok.dc_channel && v.tiktok.dc_channel.length > 1
    ).map(v => v.tiktok);
    if (!guilds) return;
  ...
  async function check(guild) {
    ...
    try {
      ...
      var channel = guild.channels.cache.get(client.social_log.get(guild.id, 'tiktok.dc_channel')) ||
        await guild.channels.fetch(client.social_log.get(guild.id, 'tiktok.dc_channel')).catch(() => { }) ||
        false;
      ...
      client.social_log.get(guild.id, 'tiktok.channels').forEach(async (tiktoker) => {
        ...
        try {
          ...
          if (!posts.collector[0]) {
            return console.log(` [TIKTOK] | ${moment().format("ddd DD-MM-YYYY HH:mm:ss.SSSS")} | ${guild.name} [${tiktoker}] :: NOT FOUND / No Posts!`.italic.brightMagenta);
          }

          const author = posts.collector[0]?.authorMeta;
          if (!author) {
            return console.log(` [TIKTOK] | ${moment().format("ddd DD-MM-YYYY HH:mm:ss.SSSS")} | ${guild.name} [${tiktoker}] :: Author metadata not found!`.italic.brightMagenta);
          }

          const allposts = posts.collector.map(p => {
            const Obj = {};
            Obj.id = p.id;
          ...
          const video = allposts[0];
          if (client.tiktok.get(tiktoker, 'oldvid') === video.id) {
            return console.log(` [TIKTOK] | ${moment().format("ddd DD-MM-YYYY HH:mm:ss.SSSS")} | ${guild.name} :: [${tiktoker}] | Last video is the same as the last saved`.italic.brightMagenta);
          }

          channel.send({
            content: client.tiktok.get(tiktoker, 'message')
