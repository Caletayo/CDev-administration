      ...
      try {
        ...
        if(channel && channel.permissionsFor(channel.guild.me).has(Permissions.FLAGS.SEND_MESSAGES)){
          ...
          collector.on('collect', async i => {
              ...
              if (!player)
                return i?.reply({content: "<:no:1249307494656901133> Nothing Playing yet", ephemeral: true})
                
              if (!channel)
                return i?.reply({
                  content: `<:no:1249307494656901133> **Please join a Voice Channel first!**`,
                  ephemeral: true
                })                  
              if (channel.id !== player.voiceChannel)
                return i?.reply({
                  content: `<:no:1249307494656901133> **Please join __my__ Voice Channel first! <#${player.voiceChannel}>**`,
                  ephemeral: true
                })
              ...
              if(i?.customId != `10` && check_if_dj(client, i?.member, player.queue.current)) {
                return i?.reply({embeds: [new MessageEmbed()
                  ...
                  .setTitle(`<:no:1249307494656901133> **You are not a DJ and not the Song Requester!**`)
              ...
              if(i?.customId == "4") {
                ...
                i?.reply({
                  ...
                  .setColor(ee.color)
                  .setTimestamp()
                  .setTitle(`${player.get(`autoplay`) ? `<:yes:1249307140682940487> **Enabled Autoplay**`: `<:no:1249307494656901133> **Disabled Autoplay**`}`)
                  .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
              }
              ...
              if(i?.customId == `6`){
                ...
                i?.reply({
                  ...
                  .setColor(ee.color)
                  .setTimestamp()
                  .setTitle(`${player.trackRepeat ? `<:yes:1249307140682940487> **Enabled Song Loop**`: `<:no:1249307494656901133> **Disabled Song Loop**`}`)
                  .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
                var data = generateQueueEmbed(client, player, track)
                swapmsg.edit(data).catch((e) => {
              ...
              if(i?.customId == `7`){
                ...
                i?.reply({
                  ...
                  .setColor(ee.color)
                  .setTimestamp()
                  .setTitle(`${player.queueRepeat ? `<:yes:1249307140682940487> **Enabled Queue Loop**`: `<:no:1249307494656901133> **Disabled Queue Loop**`}`)
                  .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
                var data = generateQueueEmbed(client, player, track)
                swapmsg.edit(data).catch((e) => {
...
/**
* @INFO
* //MODIFIED BY PHV DEVELOPMENT. | 600+ Commands Bot
* @INFO
* Read out LICENSE.MD
* @INFO
...
var embed = new MessageEmbed().setColor(ee.color)
  embed.setAuthor(`${track.title}`, "https://media.discordapp.net/attachments/1116020206154825990/1157585498957680690/save.gif", track.uri)
...
let skip = new MessageButton().setStyle('PRIMARY').setCustomId('1').setEmoji(`<:music_next:1157591921594736711>`).setLabel(`Skip`)
let stop = new MessageButton().setStyle('DANGER').setCustomId('2').setEmoji(`<:music_stop:1157587784211628084>`).setLabel(`Stop`)
let pause = new MessageButton().setStyle('SECONDARY').setCustomId('3').setEmoji('<:music_pause:1157590387423186985>').setLabel(`Pause`)
let autoplay = new MessageButton().setStyle('SUCCESS').setCustomId('4').setEmoji('<:autoplay:1157590582068269067>').setLabel(`Autoplay`)
let shuffle = new MessageButton().setStyle('PRIMARY').setCustomId('5').setEmoji('<:shuffle:1157588964308418560>').setLabel(`Shuffle`)
if (!player.playing) {
  pause = pause.setStyle('SUCCESS').setEmoji('<:music_resume:1157590857357209720>').setLabel(`Resume`)
}
if (player.get("autoplay")) {
  autoplay = autoplay.setStyle('SECONDARY')
}
let songloop = new MessageButton().setStyle('SUCCESS').setCustomId('6').setEmoji(`<:music_loop:1157587956148752475>`).setLabel(`Song`)
let queueloop = new MessageButton().setStyle('SUCCESS').setCustomId('7').setEmoji(`<:music_queue:1157589838296514573>`).setLabel(`Queue`)
let forward = new MessageButton().setStyle('PRIMARY').setCustomId('8').setEmoji('<:forward10:1157593613123977306>').setLabel(`+10 Sec`)
let rewind = new MessageButton().setStyle('PRIMARY').setCustomId('9').setEmoji('<:rewind10:1157593717348249682>').setLabel(`-10 Sec`)
